...
// inside your main loop, after BeginDrawing() but before UpdateGame logic:
void handleInputs(float Δt) {
    ...
    // keyboard input
    if (IsKeyDown(KEY_W)) dy -= moveSpeed * Δt;
    if (IsKeyDown(KEY_S)) dy += moveSpeed * Δt;

    // controller input — this must be inside game loop after drawing has started
    if (IsGamepadAvailable(0)) {
        float stickY = GetGamepadAxisMovement(0, GAMEPAD_AXIS_LEFT_Y);

        const float deadzone = 0.15f;
        if (fabsf(stickY) > deadzone) {
            dy += stickY * moveSpeed * Δt;
        }
    }
...
int main() {
    ...
    while (!WindowShouldClose()) {
      ...
      paddles[1].pos.x = w - paddles[1].size.x;
      BeginDrawing();
      drawElements(w, h, Δt);
      EndDrawing();
    }
